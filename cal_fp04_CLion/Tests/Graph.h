...
bool Graph<T>::addEdge(const T &sourc, const T &dest, double w) {
	if(findVertex(sourc)==NULL or findVertex(dest)==NULL) return false;
...
bool Graph<T>::removeVertex(const T &in) {
    ...
    for (auto &v : vertexSet) { //apaga arestas que se ligavam a temp
        for (auto &w : v->adj) {
            if(w.dest==temp)
                v->removeEdgeTo(temp);
        }
    ...
    for (auto &w : temp->adj){ //apaga arestas a que temp se liga
        temp->removeEdgeTo(w.dest);
    }
    vertexSet.erase(it);
...
vector<T> Graph<T>::bfs(const T & source) const {
    ...
    vertices.push(findVertex(source));
    findVertex(source)->visited=true;

    while(!vertices.empty()){
        Vertex<T> *temp=vertices.front();
        vertices.pop();
        res.push_back(temp->info);

        for (auto &w : temp->adj){
            if (!w.dest->visited) {
                vertices.push(w.dest);
                w.dest->visited=true;
            }
        }
    }
...
vector<T> Graph<T>::topsort() const {
    ...
    queue<Vertex<T>*> vertexs;
    for(auto &v: vertexSet){
        if(v->indegree==0)
            vertexs.push(v);
    }
    list<Vertex<T>*> vertexsT;

    while(!vertexs.empty()){
        Vertex<T>*temp=vertexs.front();
        vertexs.pop();
        res.push_back(temp->info);
        vertexsT.push_back(temp);
        for(auto &w : temp->adj){
            w.dest->indegree-=1;
    ...
    if (vertexsT.size()!=vertexSet.size()) return {};
	return res;
}
